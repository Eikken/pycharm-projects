...
@File    :   TBG_dynamic_matrix_2.py    
@Time    :   2023/2/20 17:28  
@E-mail  :   iamwxyoung@qq.com
@Tips    :   旋转矩阵的逆矩阵等于旋转矩阵的转置   inv.(U) == U.T
    角度旋转的双层声子谱
...
def fcm(*args, **kwargs):  # force constant matrix
    ...
    D = np.zeros([6 * 2, 6 * 2], dtype=np.complex)
    ...
    D[3:6, 3:6] = BB
    D[0:3, 3:6] = -DABf
    D[3:6, 0:3] = np.conj(-DABf).T  # -DBAf  # np.conj(DABs.T)
    ...
    D[9:12, 9:12] = BBs
    D[6:9, 9:12] = -DABs
    D[9:12, 6:9] = np.conj(-DABs).T  # -DBAf  # np.conj(DABs.T)
...
if __name__ == '__main__':
    ...
    angle = 0
    year = 13
    fcs = np.array(cf.force_constant_13)  # 13*52  98*166

    result = np.zeros([(30 + int(3 ** 0.5 * 10)) * n, 6 * 2])  # 解的矩阵
    ...
    for i in range((30 + int(3 ** 0.5 * 10)) * n):  # 在这里将sqrt(3)近似取为17，没有什么特别的意义
        ...
        w = list(w)
        w.sort()
        result[i, :] = (np.real(np.sqrt(w) / m ** 0.5 * 52))  # 将本征值进行保存
    s = 3 ** 0.5
    xk = [0, s, s + 1, s + 3]
