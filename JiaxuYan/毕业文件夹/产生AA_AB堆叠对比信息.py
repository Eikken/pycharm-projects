...
if __name__ == '__main__':
    ...
    a__ = 2.46
    a_cc = a__/3**0.5
    AA_layer_1, AA_layer_2 = generate_AA_AB_stack('AA', Super=Super)
    # AB_layer_1, AB_layer_2 = generate_AA_AB_stack('AB', Super=Super)

    # angle_list = [4.41, 6.01, 7.34, 9.43, 10.42, 11.64, 13.17, 15.18, 16.43, 17.90, 21.79,
    #               26.01, 27.8, 30.00]
    angle_list = [6.01]
    ...
    for i in sorted(angle_list):
        ...
        plt.scatter(AA_layer_1[:, 0], AA_layer_1[:, 1], 25, color='b')
        plt.scatter(twist_layer[:, 0], twist_layer[:, 1], 25, color='g')
        plt.scatter(tmp_init[:, 0], tmp_init[:, 1], 10, color='r')
        # plt.scatter(tmp_twist[:, 0], tmp_twist[:, 1], 30, color='y')
        plt.xticks([])
        plt.yticks([])
        plt.title('AB stack %.2f°' % i)
        plt.savefig('png/AAshow/big_atom_%.2f.png' % i)
        plt.show()
