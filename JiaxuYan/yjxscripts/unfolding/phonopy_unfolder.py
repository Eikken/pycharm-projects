...
def read_phonopy( sposcar='SPOSCAR_fcs', sc_mat=np.eye(3),force_constants=None,  disp_yaml=None, force_sets=None):
    if force_constants is None and (disp_yaml is None or force_sets is None):
        raise ValueError("Either FORCE_CONSTANTS_1213 or (disp.yaml&FORCE_SETS) file should be provided.")
    atoms=read(sposcar)
    #vesta_view(atoms)
...
def phonopy_unfold(sc_mat=np.diag([1,1,1]), unfold_sc_mat=np.diag([3,3,3]),force_constants='FORCE_CONSTANTS_1213', sposcar='SPOSCAR_fcs', qpts=None, qnames=None, xqpts=None, Xqpts=None):
...
def test():
    phonon=read_phonopy(sc_mat=np.diag([1,1,1]), force_constants='FORCE_CONSTANTS_1213', sposcar='SPOSCAR_fcs')
