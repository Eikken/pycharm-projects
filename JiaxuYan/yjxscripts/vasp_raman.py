        ...
        if "Eigenvectors after division by SQRT(mass)" in line:
            ...
            for i in range(nat*3): # all frequencies should be supplied, regardless of those requested to calculate
                ...
                for j in range(nat):
                    ...
                    #
                    eigvec.append([float(tmp[x]) for x in range(-3, 0)])
                    #
                eigvecs[i] = eigvec
                norms[i] = sqrt( sum( [abs(x)**2 for sublist in eigvec for x in sublist] ) )
            #
...
if __name__ == '__main__':
    ...
    parser.add_option('-g', '--gen', help='Generate POSCAR221205 only', action='store_true')
    parser.add_option('-u', '--use_poscar', help='Use provided POSCAR221205 in the folder, USE WITH CAUTION!!', action='store_true')
    ...
    #
    try:
        poscar_fh = open('POSCAR221205.phon', 'r')
    except IOError:
        print("[__main__]: ERROR Couldn't open input file POSCAR221205.phon, exiting...\n")
        sys.exit(1)
    #
        ...
        for j in range(len(disps)):
            ...
            try:
                ...
                if args['use_poscar'] != True:
                    print("[__main__]: File "+disp_filename+" not found, preparing displaced POSCAR221205")
                    poscar_fh = open('POSCAR221205', 'w')
                    ...
                    print("[__main__]: Using provided POSCAR221205")
                #
                if args['gen']: # only generate POSCARs
                    poscar_fn = 'POSCAR221205.%+d.out' % disps[j]
                    move('POSCAR221205', poscar_fn)
                    ...
                    if j+1 == len(disps): # last iteration for the current displacements list
                        print("[__main__]: '-gen' mode -> POSCAR221205 files with displaced atoms have been generated, exiting now")
                        sys.exit(0)
                else: # run VASP here
