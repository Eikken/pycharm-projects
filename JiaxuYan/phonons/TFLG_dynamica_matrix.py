...
@File    :   TFLG_dynamica_matrix.py    
@Time    :   2023/2/13 16:19  
@E-mail  :   iamwxyoung@qq.com
@see     :   https://journals.aps.org/prb/abstract/10.1103/PhysRevB.88.035428
@Tips    :
...
def fcm(*args, **kwargs):  # force constant matrix
    ...
    Phi_sp_2 = R_phi(np.pi/2) @ force_constant[3:6, 3:6] @ R_phi(np.pi/2).T
    Phi_sp_3 = R_phi(np.pi) @ force_constant[6:9, 6:9] @ R_phi(np.pi).T
    Phi_sp_4 = R_phi(np.arctan(rt3/5)) @ force_constant[9:12, 9:12]@ R_phi(np.arctan(rt3/5)).T
    ...
    for im in range(3):  # 2*np.pi/3
        r_ = R_phi(2 * np.pi / 3 * im) @ Phi_sp_1 @ np.linalg.inv(R_phi(2 * np.pi / 3 * im))
        kA.append(r_)
        kB.append(U @ r_ @ np.linalg.inv(U))

    # sp_ = 2 six atoms, A site
    r_ = R_phi(0) @ Phi_sp_2 @ np.linalg.inv(R_phi(0))
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = sigma_y @ R_phi(2 * np.pi / 3) @ Phi_sp_2 @ np.linalg.inv(R_phi(2 * np.pi / 3)) @ np.linalg.inv(sigma_y)
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = R_phi(2 * np.pi / 3) @ Phi_sp_2 @ np.linalg.inv(R_phi(2 * np.pi / 3))
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = sigma_y @ Phi_sp_2 @ np.linalg.inv(sigma_y)
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = R_phi(4 * np.pi / 3) @ Phi_sp_2 @ np.linalg.inv(R_phi(4 * np.pi / 3))
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = sigma_y @ R_phi(4 * np.pi / 3) @ Phi_sp_2 @ np.linalg.inv(R_phi(4 * np.pi / 3)) @ np.linalg.inv(sigma_y)
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))

    # sp_ = 3 three atoms, B site
    for im in range(3):  # 2*np.pi/3
        r_ = R_phi(2 * np.pi / 3 * im) @ Phi_sp_3 @ np.linalg.inv(R_phi(2 * np.pi / 3 * im))
        kA.append(r_)
        kB.append(U @ r_ @ np.linalg.inv(U))

    # sp_ = 4 six atoms, B site, two angle types
    r_ = sigma_y @ R_phi(np.pi / 6) @ Phi_sp_4 @ np.linalg.inv(R_phi(np.pi / 6)) @ np.linalg.inv(sigma_y)
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    r_ = R_phi(np.pi / 6) @ Phi_sp_4 @ np.linalg.inv(R_phi(np.pi / 6))
    kA.append(r_)
    kB.append(U @ r_ @ np.linalg.inv(U))
    ...
    for im in range(3):
        DAAs = DAAs + KAA[im] * np.exp(-1j * np.matmul(v_, SecA[im])) + \
               KAA[im + 3] * np.exp(-1j * np.matmul(v_, SecA[im + 3]))
        DBBs = DBBs + KBB[im] * np.exp(-1j * np.matmul(v_, SecB[im])) + \
               KBB[im + 3] * np.exp(-1j * np.matmul(v_, SecB[im + 3]))
        DABs = DABs + KAB1[im] * np.exp(-1j * np.matmul(v_, FirA[im])) + \
               KAB3[im] * np.exp(-1j * np.matmul(v_, ThiA[im])) + \
               KAB4[im] * np.exp(-1j * np.matmul(v_, FouA[im])) + \
               KAB4[im + 3] * np.exp(-1j * np.matmul(v_, FouA[im + 3]))
        DBAs = DBAs + KBA1[im] * np.exp(-1j * np.matmul(v_, FirB[im])) + \
    ...
    D[0:3, 3:6] = -DABs
    D[3:6, 0:3] = -DBAs  # p.conj(DABs.T)
    D[0:3, 0:3] = sum([sum(KAB1), sum(KAA), sum(KAB3), sum(KAB4)]) - DAAs
    D[3:6, 3:6] = sum([sum(KAB1), sum(KAA), sum(KAB3), sum(KAB4)]) - DBBs
