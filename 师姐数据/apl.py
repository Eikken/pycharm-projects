...
def draw_line(**kwargs):
    ...
    can = kwargs['c']
    fn = kwargs['fname']
    plt.plot(tM[:, 0], tM[:, 1] + can * 70, label=fn)
...
if __name__ == '__main__':
    t1 = time.time()
    filePath = r'C:\Users\Celeste\Desktop\apl'  # 文件的根目录
    fNames = os.listdir(filePath)
    xyFileName = []
    for i in range(25):
        angle = i * 15
        xyFileName.append(['%d 1 (X-Axis).txt' % angle, '%d 1 (Y-Axis).txt' % angle])
    dataSet = np.zeros((1024, 50))
    smoothData = np.zeros((1024, 26))
    inDx = 0
    ...
    for i in range(25):
        ...
        draw_Smooth(xy=thisMatrix, c=i, fname=str(i * 15))
